...
export interface I_MicroDOM extends Array<Element> {
  get(...args: string[] | Element[]): I_MicroDOM;                   // Returns a new instance with new elements from each element in the array (or from the document if there are no current elements)
